...
Config.declareTypes(["Attachment Locking Channel", "Ticket Transcripts Channel"])
...
export default class TicketTranscriber {
    ...
    async generateHTMLTranscript(ticket: Ticket, guild: Guild, channel?: TextChannel | null) {
        ...
        const transcriptContent = await discordTranscripts.generateFromMessages(messages, channel, {
    ...
    async send(guild: Guild, ticket: Ticket, channel?: TextChannel | null) {
        ...
        const channelId = this.bot.getConfigValue(`Ticket Transcripts Channel`, guild.id)
        if (channelId) {
